...
def install_toggle_script():
    ...
    try:
        ...
        esc_plugin = shlex.quote(plugin_path)
        esc_config = str(CONFIG_FILE).replace("'", "'\\''")
...
PYEOF
    ;;
  dashboard)
    python3 {esc_plugin} --dashboard
    ;;
  force-update)
    python3 {esc_plugin} --force-update
    ;;
esac
""")
